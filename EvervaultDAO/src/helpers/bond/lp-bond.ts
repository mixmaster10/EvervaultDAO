...
export class LPBond extends Bond {
    ...
    public getEveAmount(networkID: Networks, provider: StaticJsonRpcProvider) {
    ...
    private async getReserves(networkID: Networks, provider: StaticJsonRpcProvider, isToken: boolean): Promise<number> {
        ...
        const isEve = token1.toLowerCase() === addresses.EVE_ADDRESS.toLowerCase();

        return isToken ? this.toTokenDecimal(false, isEve ? reserve0 : reserve1) : this.toTokenDecimal(true, isEve ? reserve1 : reserve0);
    }

    private toTokenDecimal(isEve: boolean, reserve: number) {
        return isEve ? reserve / Math.pow(10, 9) : reserve / Math.pow(10, 18);
    }
}
